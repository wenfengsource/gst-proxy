...
on_new_sample_from_sink (GstElement * elt, ProgramData * data)
{
 ...
 source2 = gst_bin_get_by_name (GST_BIN (data->sink2), "testsource2");
...
main (int argc, char *argv[])
{
 ...
 GstElement *testsource2 = NULL;
  ...
  string =
      g_strdup_printf
      ("gst-launch-1.0 -v udpsrc uri=udp://0.0.0.0:1234 caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, 			payload=(int)33\" ! appsink name=testsink");
  //g_free (filename);
   ...
   * memory. */
  string =
      g_strdup_printf ("appsrc name=testsource caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, 			payload=(int)33\" ! rtpmp2tdepay ! filesink location=./test.ts");
 ...
 string =
      g_strdup_printf ("appsrc name=testsource2 caps=\"application/x-rtp, media=(string)video, clock-rate=(int)90000, 			payload=(int)33\" ! udpsink host=192.168.128.151 port=1024");
  data->sink2 = gst_parse_launch (string, NULL);
  g_free (string);
...
testsource2 = gst_bin_get_by_name (GST_BIN (data->sink), "testsource2");
  /* configure for time-based format */
  g_object_set (testsource2, "format", GST_FORMAT_TIME, NULL);
  /* uncomment the next line to block when appsrc has buffered enough */
  /* g_object_set (testsource, "block", TRUE, NULL); */
  gst_object_unref (testsource2);
