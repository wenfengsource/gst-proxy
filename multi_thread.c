...
void for_each_all_callid(gpointer key, gpointer value, gpointer user_data);
...
//		break;
	...
	case GST_MESSAGE_ERROR:{
 	  ...
 	  if(pt !=NULL)
	  {
			...
			sink = g_hash_table_lookup (gst_ptr->sink_hashtable, pt+14);
			if(sink == NULL)
			{
				g_free (ele_name);
				break;
...
//			g_mutex_unlock (&snd_data_mutex);
	...
	if(pt1 != NULL) // multiudpsink
	{
		...
		sink = g_hash_table_lookup (gst_ptr->sink_hashtable, pt1+13);
		if(sink == NULL)
		{
			g_free (ele_name);
			break;
	...
	//	 printf("%s error message \n", pt1);
		...
		g_mutex_lock (&snd_data_mutex);
		bzero(tx_buf,sizeof(tx_buf));
		sprintf(tx_buf, "self;callid=%s;sipuri=%s;bye=ok;",pt1+13, gst_ptr->sipuri);
		send_packet(tx_buf,strlen(tx_buf),"0.0.0.0",LISTEN_PORT);
		g_mutex_unlock (&snd_data_mutex);
...
//		g_mutex_unlock (&snd_data_mutex);
	...
	{
		sink = g_hash_table_lookup (gst_ptr->sink_hashtable, pt2+14);
		if(sink == NULL)
		{
			g_free (ele_name);
			break;
		...
		g_mutex_lock (&snd_data_mutex);
		bzero(tx_buf,sizeof(tx_buf));
		sprintf(tx_buf, "self;callid=%s;sipuri=%s;bye=ok;",pt2+14, gst_ptr->sipuri);
		send_packet(tx_buf,strlen(tx_buf),"0.0.0.0",LISTEN_PORT);
		g_mutex_unlock (&snd_data_mutex);
...
//		g_mutex_unlock (&snd_data_mutex);
...
#if 1
	   // src error
		pt = strstr( ele_name ,"rtspsrc");
	   if(pt != NULL)
	   {
		   	g_mutex_lock (&gst_ptr->sink_hash_mutex);
			 if(gst_ptr->sink_hashtable !=NULL)
			 g_hash_table_foreach(gst_ptr->sink_hashtable, for_each_all_callid, NULL);
    ...
    case GST_MESSAGE_WARNING:{
      ...
      // some times , rtsp server close , not occur GST_MESSAGE_EOS signal ,so we need close here
      g_error_free (err);
      g_free (debug);
      g_free (name);
      break;
...
//  }
    ...
    {
    	g_print (" audio pad\n");
    	if(g_audio_codec ==CODEC_AAC || g_audio_codec == CODEC_PCMULAW)
    	{
                                  ...
                                  gpointer user_data)
{
  ...
  //GST_RTSP_FILTER_REF,
     ...
     char *ptr = (char *)user_data;
	  ...
	  //priv->server_ip;
    // printf("remove ptr= %s client ===path %s  %p\n", ptr, gst_rtsp_client_get_path(client), client);
	   if(strstr(gst_rtsp_client_get_path(client),ptr) !=NULL)
	   {
 ...
 int Create_source_sink_pipeline( GstCustom *gstcustom)
{
	 ...
	 {
	 	gstcustom->pipeline = gst_pipeline_new ("rtsp");
	 }
	 else
	 {
	...
	if(gstcustom->source.type == UDP || gstcustom->source.type == RTP)
	{
		...
		if((strstr(gstcustom->source.keep_alive_str, "ccff") != NULL) || (strstr(gstcustom->source.keep_alive_str, "CCFF") != NULL)
				|| gstcustom->source.keep_alive_str[0] == 0)
		{
			g_object_set (gstcustom->source.src, "keep-alive-len", 10, NULL);
			g_object_set (gstcustom->source.src, "keep-alive-string","ccff",NULL);
		}
		else
		{
	...
	{
		    if(strstr(gstcustom->source.rtspaddr,"mpeg4"))
		    {
				...
				{
					gstcustom->source.aacparse = gst_element_factory_make ("mpegaudioparse", "mpegaudioparse");
					gstcustom->source.audiodepay = gst_element_factory_make ("rtpmpadepay", "rtpmpadepay");
...
//
		...
		if((invite_flag ==1) && (sink_dst_ip_parse(rx_buf, rcv_size,sink_dst_ip) == 0) && sink_type != RTSP)
		{
       ...
       // sleep(1);
		if(invite_flag == TRUE)
		{
			...
			case TCPSERVER:
			{
				...
				{
					if(sink_type == UDP || sink_type == TCPSERVER || sink_type == TCPCLIENT || sink_type == RTSP)
					{
				...
				//		  printf("+++++++++++++source_src_ip = %s \n", source_src_ip);
					    ...
					    else if(sink_type == RTSP)
						{
							//rtsp_server_uri
							// g_stpcpy(gst_ptr->sink->src_ip, sink_src_ip);
							 // accord rtsp_server_uri set udp dst port,
							 // check is has rtsp server uri, if had , set bye first
							// if()
							//bzero(rtsp_server_uri,sizeof(rtsp_server_uri));
							 ...
							 if(port >=0 && port < g_rtsp_channel_max_cnt)
							 {
								 ...
								 g_stpcpy(gst_ptr->sink->dst_ip, "0.0.0.0");
								 gst_ptr->sink->dst_port = 1600+port;
								 gst_ptr->sink->type = UDP;
						...
						if(sink != NULL)
					    {
							...
							if(g_hash_table_size(tmp->sink_hashtable) > 1)
							{
								...
								//printf("sink tee pad address =%p ,callid =%s\n", sink->teepad, callid);
								 ...
								 g_mutex_lock (&snd_data_mutex);
								 bzero(tx_buf,sizeof(tx_buf));
								 sprintf(tx_buf,"code=1002;sipuri=%s;callid=%s;port=0;",tmp->sipuri,tmp->sink->callid);
								 send_packet(tx_buf,strlen(tx_buf),g_remote_ip,SND_PORT);
								 g_mutex_unlock (&snd_data_mutex);
...
void free_all_keepalive_for_sink(gpointer key, gpointer value, gpointer user_data)
 ...
 void for_each_all_callid(gpointer key, gpointer value, gpointer user_data)
 {
	 ...
	 g_mutex_lock (&snd_data_mutex);
	 bzero(tx_buf,sizeof(tx_buf));
	 sprintf(tx_buf, "self;callid=%s;sipuri=%s;bye=ok;",sink->callid, sink->sipuri);
...
//	g_mutex_unlock (&tmp->sink_hash_mutex);
 ...
 //   usleep(5000); // here only 5ms
	...
	// printf("exit loop 11111  gstdata->sink_hashtable %p \n"  ,gstdata->sink_hashtable );
	g_mutex_lock (&gstdata->sink_hash_mutex);
	 if(gstdata->sink_hashtable !=NULL)
...
//	printf(" len=%d, buf=%s  add=%p\n", len, buf, src_address);
 ...
 void create_keep_alive_socket_for_sink(Sink *sink)
 {
	 ...
	 {
		 //Create socket for receive data
		 printf("unnecessary create socket by self \n");
		 return;
	 }
 ...
 //   Hashtbl_Tcp_sink_snd_port = g_hash_table_new_full (g_str_hash , g_str_equal ,free_tcp_snd_port_key,print_port_value);
   ...
   // add_source(NULL);
    ...
    for( i=0; i<g_rtsp_channel_max_cnt ; i++)
    {
    	...
    	memset(uri,0,30);
    	sprintf(uri,"udp://0.0.0.0:%d",i+1600);
 		//factory[i] = gst_rtsp_media_factory_uri_new ();
 		// gst_rtsp_media_factory_uri_set_uri (factory[i], uri);

 		//gst_rtsp_media_factory_set_shared(factory[i],1);

		// gst_rtsp_mount_points_add_factory (mounts,  rtsp_uri, GST_RTSP_MEDIA_FACTORY (factory[i]));
